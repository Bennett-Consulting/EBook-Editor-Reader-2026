...
// EPUB
// =========================================================================
async function buildEpub(book: Book): Promise<Blob | string> {
  const zip = new JSZip();
...
// =========================================================================
// DOCX  (built manually as a minimal Open XML zip)
// =========================================================================
...
async function buildDocx(book: Book): Promise<Blob | string> {
  const zip = new JSZip();
  zip.file(
    "[Content_Types].xml",
...
async function buildPdfWeb(book: Book): Promise<void> {
  // Use the browser's native print → "Save as PDF" via a hidden iframe.
...
async function buildPdfNative(book: Book): Promise<string> {
  const html = buildPrintableHtml(book);
  // expo-print returns a uri to a real PDF file generated by the OS
...
export async function exportBook(book: Book, format: ExportFormat) {
  ...
  if (Platform.OS === "web") {
    try {
      ...
      if (format === "epub") {
        blob = (await buildEpub(book)) as Blob;
      } else if (format === "docx") {
        blob = (await buildDocx(book)) as Blob;
      } else if (format === "pdf") {
        await buildPdfWeb(book);
        return; // browser print dialog handles save-as-PDF
      } else {
  ...
  try {
    ...
    if (format === "epub") {
      const base64 = (await buildEpub(book)) as string;
      await FileSystem.writeAsStringAsync(uri, base64, {
        encoding: FileSystem.EncodingType.Base64,
      });
      mime = "application/epub+zip";
    } else if (format === "docx") {
      const base64 = (await buildDocx(book)) as string;
      await FileSystem.writeAsStringAsync(uri, base64, {
        encoding: FileSystem.EncodingType.Base64,
      });
      mime =
      ...
      // expo-print returns its own file path; reuse it directly
      uri = await buildPdfNative(book);
